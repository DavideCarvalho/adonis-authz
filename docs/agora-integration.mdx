...
<Callout type="warn">
  **Renamed in a recent release.** `roleGrants` was formerly `globalRoleGrants`.
  The name changed because the map now applies to the user's *effective* roles
  (context ∪ `resolveRoles` ∪ store), not just global ones. The provider reads
  **only** `roleGrants` — an old `globalRoleGrants` key is silently ignored, so
  rename it. See [Effective roles](/docs/authz/config#effective-roles-resolveroles--rolegrants)
  for the full model.
</Callout>
