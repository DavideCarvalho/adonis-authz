...
description: How @agora/authz registers static abilities backed by the store, and how to use them in controllers and Edge.
icon: Shield
---
...
no runtime API to register one ability per database row. So `@agora/authz`
...
import { AuthzService, defineAuthzAbilities } from '@agora/authz'
...
<Callout type="info">
  ...
  pair `@agora/authz` with a normal Bouncer policy and combine the checks.
</Callout>
